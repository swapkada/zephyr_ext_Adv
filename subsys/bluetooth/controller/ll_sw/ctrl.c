			     ...
			     u16_t lazy, void *context)
{
	...
	event_common_prepare(ticks_at_expire, remainder,
			     ...
			     &_radio.advertiser.hdr.ticks_active_to_start,
			     _radio.advertiser.hdr.ticks_preempt_to_start,
			     RADIO_TICKER_ID_EXT_ADV, event_ext_adv, NULL);
		      ...
		      void *context)
{
	...
	if(aux_scan){
		...
		if(Time_difference_ticker < _radio.ext_scanner.chan_off){
			_radio.ext_scanner.chan_off_calculation = _radio.ext_scanner.chan_off_calculation - Time_difference_ticker;
		}
...
u32_t radio_adv_enable(u8_t phy_p, u16_t interval, u8_t chan_map,
		       u8_t filter_policy, u8_t rl_idx, u8_t max_skip)
#else /* !CONFIG_BT_CTLR_ADV_EXT */
...
#endif /* !CONFIG_BT_CTLR_ADV_EXT */
{
	...
	if (_radio.advertiser.is_hdcd) {
		...
		u16_t size, final_pckt_size;
		u32_t ticks_now = ticker_ticks_now_get();
		u8_t skip = max_skip;						//number of events skip before transmitting Aux packet on data channel
		...
		_radio.ext_advertiser.aux_pckt_time = ((((u64_t)interval * 625 ) ) * skip) + 10000;
		...
		size = pdu->len + 2;
		final_pckt_size = (size + 1 + 4 + 3)*8 ; 			//preamble + Access Address + CRC + size of pdu
		...
		/* ticker start for the primary packts on advertsing channels */
		ret =
			ticker_start(RADIO_TICKER_INSTANCE_ID_RADIO,
				     RADIO_TICKER_USER_ID_APP,
				     RADIO_TICKER_ID_ADV,
		...
		/*ticker start for secondary packets(Aux packets) on data channels */
